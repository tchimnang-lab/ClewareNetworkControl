...
//
// (C) 2001-2023 Copyright Cleware GmbH
// All rights reserved
//
// History:
...
// 18.02.2018	5.1.3	ws	added PT100 and PT1000 support, fixed Cutter 3.0 detection in isCutter function
// 12.03.2018	5.1.4	ws	added USB-Color support
// 19.04.2018	5.1.5	ws	added new USB-Humi support
// 31.05.2018	5.1.6	ws	FCWinit - internal object CUSBaccess now static to avoid C# memory leaks, GetTemperature start problem fixed
// 17.09.2018	5.1.7	ws	ADC now 3 channel device
// 08.11.2018	5.1.8	ws	Adjust KnoxBox fan speed
// 12.11.2018	5.1.9	ws	Add CleWlan support
// 12.11.2018	5.2.0	ws	Missing FCW function IsMonitoredSwitch added
// 29.04.2019	5.2.1	ws	Added new humidity sensor support
// 11.07.2019	5.2.2	ws	in rare cases, USB-Temp return time==0 && value==0 - this is an error
// 04.09.2020	5.2.3	ws	ForisControl got a new SHT35 sensor
// 05.11.2020	5.2.4	ws	Add USB-CO2 device type, support will follow on demand
// 10.12.2020	5.2.5	ws	Support for new Ampel4 type, which allows indipendant control of all light 
// 12.03.2021	5.2.6	ws	make C-Interface more ease to to use with (x64) python, introduces FCDIGet...
// 30.06.2021	5.2.7	ws	fix GetCounter, added SetCounter to FCW interface
// 07.07.2021	5.2.8	ws	fix SetCounter bug
// 31.08.2021	5.2.9	ws	add FCWGetContact for easy access
// 13.01.2022	5.3.0	ws	add ADC_I2C type, File Version visible again
// 20.01.2023	5.3.1	ws	ADC: fetch factor and delta and use this in GetTemperature to handle PT100 devices
...
const int USBaccessVersion = 531 ;
...
class USBACCESS_API CUSBaccess {
	public:
		...
		enum ADCTypes {			ADC_HX71=1, ADC_PT100=2, ADC_PT1000=3, ADC_121=5, ADC_I2C=6  } ;		// ADC_121 is the standard ADC device
		...
		enum USBtype_enum {		ILLEGAL_DEVICE=0,
								...
								COLOR_DEVICE=0xa0, COLOR_RD_DEVICE = 0xa1, COLOR_GN_DEVICE = 0xa2, COLOR_BL_DEVICE = 0xa3,	// Color Sensor, returns white and r g b
	...
	public:
		...
		virtual int			GetContact(int deviceNo) ;	// returns (mask << 16) + value
		...
		virtual int			IsCutter(int deviceNo) ;		// return true if this is a cutter device 1=Cutter, 2=Connect, 3=Multi2, 4=Multi2X, 17=Cutter3.0, 18=Connect3.0
		...
		virtual int			IsIdeTec(int deviceNo) ;			// for internal use only
		...
		virtual int			GetColor(int deviceNo, int *count_white, int *count_red, int *count_green, int *count_blue) ;
...
enum FCWADCTypes {			ADC_HX71=1, ADC_PT100=2, ADC_PT1000=3, ADC_121=5  } ;		// ADC_121 is the standard ADC device
...
// functional C interface (FCW = Function CleWare)
 ...
 extern "C" {
	...
	USBACCESS_API int _stdcall			FCWIGetTemperature(CUSBaccess *obj, int deviceNo) ;		// returns value*1000 as int
	...
	USBACCESS_API int _stdcall			FCWIGetHumidity(CUSBaccess *obj, int deviceNo) ;		// returns value*1000 as int
	...
	USBACCESS_API int _stdcall			FCWIGetADC(CUSBaccess *obj, int deviceNo, int sequenceNumber, int subDevice) ;		// returns value*1000 as int
	...
	USBACCESS_API int _stdcall			FCWGetContact(CUSBaccess *obj, int deviceNo) ;	// returns (mask << 16) + value
	...
	USBACCESS_API int _stdcall			FCWGetCounter(CUSBaccess *obj, int deviceNo, int counterID) ;
	USBACCESS_API int _stdcall			FCWSetCounter(CUSBaccess *obj, int deviceNo, int counter, int counterID) ;
	...
	USBACCESS_API int _stdcall			FCWIsIdeTec(CUSBaccess *obj, int deviceNo) ;			// return true if watchdog inverted
	USBACCESS_API int _stdcall			FCWIsMonitoredSwitch(CUSBaccess *obj, int deviceNo) ;	// return true if this is a monitored switch
